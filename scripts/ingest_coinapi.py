...
# --- Wrapper para ingestar orderbook por fecha ---
def process_orderbook_for_date_task(symbol, date_str):
    """
    Wrapper que abre su propia conexión a SQLite y llama a ingest_orderbook.
    """
    logs = []
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    cur = conn.cursor()
...
def fetch_orderbook_5min(symbol: str, date: str) -> pd.DataFrame:
    ...
    # Si no hay snapshots, devolver DataFrame vacío para que ingest_orderbook inserte marcador
    if not records:
        return pd.DataFrame()
...
def process_symbol(symbol_row):
    ...
    # Determinar rango de fechas desde data_start hasta ayer UTC
    start_date = datetime.fromisoformat(start_str).date()
    stored_end = datetime.fromisoformat(end_str).date()
    today_minus1 = datetime.now(timezone.utc).date() - timedelta(days=1)
    end_date = min(stored_end, today_minus1)

    # Detectar días faltantes en coinapi_orderbook
    pending_dates = []
    curr = start_date
    while curr <= end_date:
        cur.execute(
    ...
    # Ingestar snapshots para cada día faltante
    for date_str in pending_dates:
        try:
            logs.extend(ingest_orderbook(symbol, date_str, conn, cur))
        except Exception as e:
            logs.append(f"[Orderbook] Error en {symbol} {date_str}: {e}")
            continue

    conn.close()
    logs.append(f"[Orderbook] Completed {len(pending_dates)} days for {symbol}")
    return logs
